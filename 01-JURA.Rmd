...
library(kableExtra)
fr <- data.frame(
  Aktivitet = c(rep("Anbringelse", 1), rep("Sløring",1 ),rep("Anvendelse",1)),
  Forklaring = c("Det ulovlige udbytte anbringes, Det kan fx. være i det finansielle system.", "Det ulovlige udbytte adskilles fra dets kilde. Det kan f.eks. ske gennem (finansielle) transaktioner.", "Udbytte tilbageføres til gerningsrmanden. Det kan f.eks. være i en form, hvor udbyttet er ændret til midler eller aktiver, der ser ud til at være lovlige."),
  EksempelA = c(
...
names(fr) <- c("Aktivitet" , "Forklaring" ,"Eksempel 1" , "Eksempel 2" , "Eksempel 3")
kable(fr) %>%
  kable_styling(full_width = T,font_size = 10,bootstrap_options = c("responsive","bordered","striped")) %>%
  column_spec(1, bold = T, border_right = T, color = "white", background = "lightblue") %>%
  row_spec(0,italic = T, color = "white", background = "lightblue",font_size = 12) %>%
  # column_spec(0, angle = -45)%>%
  # scroll_box(width = "500px") %>%
